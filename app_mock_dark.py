...
app.layout = DarkThemeProvider([
    ...
    html.Div([
        ...
        dcc.Tabs(
            ...
            value=1,
            id='tabs',
            style={'backgroundColor': '#EBF0F8', 'color': '#2a3f5f', 'height': '80%'},
        ),

        html.Div([
            html.Div([
                         html.Div([
                            html.Div(id="graph_info", style = {'textAlign': 'center', 'fontSize': '16px', 'padding': '0px 5px', 'lineHeight': '20px', 'border': '2px solid #EBF0F8'}),
                         ], className="row graph-param"),
                ], className="six columns"),
            html.Button('+',
                        id='new_tab',
                        type='submit',
                        style={'backgroundColor': '#EBF0F8', 'height': '20px', 'width': '20px',
        ...
        dcc.Graph(
            ...
            figure=dict(
                data=[dict(x=np.linspace(-0.000045, 0.000045, 1e3),
                           ...
                           marker={'color': '#f2f5fa'})],
                layout=go.Layout(
                    xaxis={'title': 's', 'color': '#EBF0F8', 'titlefont': dict(
                        family='Dosis',
                        size=15,
                    )},
                    yaxis={'title': 'Voltage (V)', 'color': '#EBF0F8', 'titlefont': dict(
                        family='Dosis',
                        size=15,
...
@app.callback(Output('tabs', 'style'),
              [Input('color_picker', 'value')])
def color_tabs_background(color):
    return {'backgroundColor': color['hex'], 'color': '#2a3f5f'}
...
def update_output(value, frequency, wave, amplitude, offset, osc_on, fnct_on):
    ...
    if not osc_on:
        return dict(
            data=[],
            layout=go.Layout(
                xaxis={'title': 's', 'color': 'rgba(0,0,0,0)', 'titlefont': dict(
                    family='Dosis',
                    size=15,
                ), 'showticklabels': False, 'ticks': '', 'zeroline': False},
                yaxis={'title': 'Voltage (V)', 'color': 'rgba(0,0,0,0)', 'titlefont': dict(
                    family='Dosis',
                    size=15,
